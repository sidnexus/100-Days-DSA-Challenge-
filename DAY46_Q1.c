...
int main() {
    ...
    struct Queue output; output.front = 0; output.rear = -1;
    if(root) enqueue(&output, root);
    while(!isEmpty(&output)) {
        struct Node* curr = dequeue(&output);
        printf("%d ", curr->data);
        if(curr->left) enqueue(&output, curr->left);
        if(curr->right) enqueue(&output, curr->right);
    }
    return 0;
}
